...
$(document).ready(function () {
  ...
  function saveCanvas(event) {
    $("#signature_error").text("");
    $("#loader").show();
    saveButton.disabled = true; // Disable save button to prevent double click
    ...
    if (isCanvasBlank) {
      $("#loader").hide();
      saveButton.disabled = false; // Enable save button again
      $("#signature_error").text(
    ...
    // Check if policy content is visible
    if (element.offsetParent === null) {
      $("#loader").hide();
      saveButton.disabled = false;
      alert("Please wait while the previous document is being processed.");
      return;
    }
      ...
      .then(function (pdfBlob) {
        ...
        $.ajax({
          ...
          success: function (response) {
            $("#loader").hide();
            saveButton.disabled = false; // Enable Save button back
            ...
            if (response.success) {
              ...
              clearCanvas(); // <-- Now clear canvas AFTER successful upload and DOM updates
            } else {
  ...
  // Event listeners for canvas drawing (desktop and mobile)
