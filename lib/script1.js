			...
			<div class="items_list" style="background-color: #1b416f;">
				...
				<div class="items_config">
					...
					<h5 class="items_config_title" data-OrderCount="" style="color: white;">
						${formatCurrencyCustom(setupPrice)} 
					</h5>
				</div>
			</div>
		`;
	...
	$(document).ready(function() {
        ...
        $('.get_hard').click(function() {
			...
			if (targetDiv.find('.set_prd_name').hasClass('first_prd_name')) {
				...
				titleHtml = `<span>${getPrdText}</span><span  data-optIdLeftBar="${dataId}">${numeral(newPackegesPrice).format('0,0.00')}</span>`;
				...
				const newModification1 = `
					<div class="items_list">
						...
						<div class="items_config">
							...
							<h5 class="items_config_title"  >${index==0 ? numeral(newPackegesPrice).format('$0,0.00'):numeral(getPriceText).format('$0,0.00')}
							</h5>
						</div>
					</div>
				`;
...
// Get the updated quantity and append it to the order summary
		...
		// Set the set_title span content
		const setTitle = $('.summary-item'),
		titleHtml = `<span>${getPrdText}</span><span  data-optIdLeftBar="${dataOptId}">$${numeral(newPackegesPrice).format('0,0.00')}</span>`;
		setTitle.empty(); // Clear any existing content
		setTitle.html(titleHtml);
		...
		$('.get_all_prd_info').each(function(index) {
			...
			const newModification1 = `
				<div class="items_list">
					...
					<div class="items_config">
						<h5 class="items_config_title ">${getPrdText}</h5>
						<h5 class="items_config_title"  >${index==0 ?  numeral(newPackegesPrice).format('$0,0.00'): numeral(getPriceText).format('$0,0.00')} </h5>
					</div>
				</div>
			`;
			// Append the new Order summary to the Order summary section
		...
		$('#refresh_product_options').click(function () {
			...
			const newPackegesPrice=priceObject.packRec;
			$('.order_summary_title').empty();
			// Reset all product options to their default values
			$('.get_all_prd_info').each(function (index) {
				const targetDiv = $(this); // Target div to reset
		
				// Find the first option within the dropdown data
				const firstOption = targetDiv.closest('.items_list').find('.dropdown_data li:first-child');
				const getNameText = $(this).closest('.items_list').find('.items_name h5').text();
		
				if (firstOption.length > 0) {
					// Get default values from the first option
					...
					// Reset the input field to default attributes
					...
					// Reset the displayed product name and price
					targetDiv.find('.set_prd_name').html(defaultPrdName || '');
					targetDiv.find('.set_price_html').html(defaultPrice);
					...
					const newModification1 = `
						<div class="items_list">
							...
							<div class="items_config">
								...
								<h5 class="items_config_title"  >${index==0 ?  numeral(newPackegesPrice).format('$0,0.00'): numeral(defaultPrice).format('$0,0.00')} </h5>
							</div>
						</div>
			`;
			// Append the new Order summary to the Order summary section
		...
		function clearCanvas() {
			if (signatureInput.value) {
				signatureInput.value = ''; // Set to empty
			}
		...
		function saveCanvas(event) {
			...
			if (isCanvasBlank) {
				$('#loader').hide();
				$('#signature_error').text('Please provide a signature before proceeding.');
					...
					success: function (response) {
						...
						if (response.success) {
							...
							if (dataId == 1) {
								...
								let storedFiles = JSON.parse(localStorage.getItem('filePaths')) || []; // Retrieve existing 
								storedFiles.push(filePath); // Add new file path
								localStorage.setItem('filePaths', JSON.stringify(storedFiles)); // Save updated array back 
							} else {
								let storedFiles = JSON.parse(localStorage.getItem('filePaths')) || []; // Retrieve existing 
								storedFiles.push(filePath); // Add new file path
								localStorage.setItem('filePaths', JSON.stringify(storedFiles)); // Save updated array back 
...
//  refresh 
		...
		$('.submitButton').on('click', function(e) {
			...
			if (!termConditionChecked) {
				$('#term_condition_error').text('You must agree to the terms and conditions.').show();
		...
		$('refresh_product_options').on('click',function(e){
			e.preventDefault();
			$('div.changed_div').removeClass('changed_div');
			$('.all-modifications-section, .first-modifications-section').empty(); // Clear any existing content
			$('.order_summary_title').empty();
		...
		$(".payswitch").click(function(){
			...
			if(!$(this).hasClass("pay-icon-selected")){
				...
				$(".payswitch").each(function() {
					if ($(this).hasClass("pay-icon-selected")) {
						$(this).css("background-color", "#ffc439"); // Example color for selected
					} else {
...
// validation of expiry Years of card
	...
	function validateYear() {
		...
		if (expiryYear < currentYear || (expiryYear === currentYear && parseInt(document.getElementById('ccm').value.trim()) < currentMonth)) {
