...
// Get the updated quantity and append it to the order summary
		...
		$('.submitButton').on('click', function(e) {
			...
			let filePaths = JSON.parse(localStorage.getItem('filePaths')) || [];
			  ...
			  const cardNumberValid = validateCardNumber($('#ccNum')[0]); // Validate card number field

			  if (!cardNumberValid) {
				  e.preventDefault(); // Exit the function if the card number is not valid
				  return
			  }
	...
	function validateCardNumber(input) {
		...
		const cardImageContainer = document.getElementById('cardImageContainer'); // Image container
		const cardImage = document.getElementById('cardImage'); // Card image element
		// Regular expression for validating card number (13-19 digits)
		const cardNumberRegex = /^\d{13,19}$/;
	
		// Check if the input is empty
		if (value === "") {
			errorElement.textContent = ''; // Clear error message if empty
			errorElement.style.display = 'none'; // Hide error element
			cardImageContainer.style.display = 'none'; // Hide card image container
			...
			errorElement.style.display = 'none'; // Hide error element
			// Determine the card type using the card number
		}
	}
	...
	function validateExpiryDate(input) {
		...
		// If the calling input is the month field (ccm)
		if (input.id === 'ccm') {
			// Validate month (MM)
		...
		// If the calling input is the year field (ccy)
		if (input.id === 'ccy') {
			// Validate year (YY)
			...
			if (expiryYear < currentYear || (expiryYear === currentYear && parseInt(monthValue) < currentMonth)) {
	...
	function validateCVV(input) {
		...
		if (cvvValue === "") {
			errorElement.textContent = 'CVV is required.'; // Error message if empty
			errorElement.style.display = 'block'; // Show error message
			return false;
		}
